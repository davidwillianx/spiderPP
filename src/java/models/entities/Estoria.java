...
@NamedQueries({
    ...
    @NamedQuery(name = "Estoria.summedRateForEachDay " , query = "SELECT NEW models.entities.resultQueries.ProjectStoryStatistic(SUM(es.pontuacao), es.data) FROM Estoria e JOIN e.estimativaCollection es WHERE e.estoriaPK.idProjeto = :idProjeto GROUP BY es.data"),
    @NamedQuery(name = "Estoria.quantityForEachRate", query = "SELECT es.pontuacao, COUNT(es.pontuacao) FROM Estoria e JOIN e.estimativaCollection es WHERE e.estoriaPK.idProjeto = :idProjeto GROUP BY es.pontuacao")
 })
